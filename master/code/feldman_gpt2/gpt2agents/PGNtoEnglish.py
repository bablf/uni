...
from typing import List, Tuple, TextIO, Pattern
...
class Move:
    ...
    def set_expanded(self, source: Tuple, destination: Tuple, piece: PIECES, is_white: bool, meta_list=None):
        ...
        if random.random() > 0.25:
            use_name = False
            if is_white:
                p = piece.value.replace("white ", "")
                text = "White moves {} from {}{} to {}{}.".format(p, source[0], source[1], destination[0],
                                                                  destination[1])
            else:
                p = piece.value.replace("black ", "")
                text = "Black moves {} from {}{} to {}{}.".format(p, source[0], source[1], destination[0],
                                                                  destination[1])
        for m in meta_list:
            text += " {}".format(m)
        if is_white:
    ...
    def to_narrative(self) -> str:
        ...
        for c in self.comment_list:
            s += "{}\n".format(c)
        if 'unset' in s:
            dprint("Move.to_narrative(): Problem with {}".format(s))
            dprint(self.to_string())
        return s
...
class Game:
    ...
    def to_narrative(self, msi: MSI.MySqlInterface = None) -> str:
        s = "On {}, {} played {}. ".format(self.match_date.strftime("%B %d, %Y"), self.white_player, self.black_player)
        if self.match_date.day == 1 and self.match_date.month == 1:
            s = "In {}, {} played {}. ".format(self.match_date.year, self.white_player, self.black_player)

        if self.white_elo_rating > self.black_elo_rating:
            s += "{} was the higer-ranked player, with an Elo rating of {}. ".format(self.white_player,
                                                                                     self.white_elo_rating)
            if self.black_elo_rating == -1:
                s += "{} was unrated. ".format(self.black_player)
            else:
                s += "{} was the lower-ranked player, with an Elo rating of {}. ".format(self.black_player,
                                                                                         self.black_elo_rating)
        elif self.white_elo_rating < self.black_elo_rating:
            s += "{} was the higer-ranked player, with an Elo rating of {}. ".format(self.black_player,
                                                                                     self.black_elo_rating)
            if self.black_elo_rating == -1:
                s += "{} was unrated. ".format(self.white_player)
            else:
                s += "{} was the lower-ranked player, with an Elo rating of {}. ".format(self.white_player,
                                                                                         self.white_elo_rating)
        elif self.black_elo_rating < 0:
            s += "Both players were unranked. "
        else:
            s += "Both player had the same Elo rating. "

        if self.result == '1-0' and (self.white_elo_rating < self.black_elo_rating):
            s += "{} won in a surprise victory over {} in a game that lasted {} moves. ".format(self.white_player,
                                                                                                self.black_player,
                                                                                                len(self.move_list))
        elif self.result == '0-1' and (self.white_elo_rating > self.black_elo_rating):
            s += "{} won in a surprise victory over {} in a game that lasted {} moves. ".format(self.black_player,
                                                                                                self.white_player,
                                                                                                len(self.move_list))
        elif self.result == '1-0':
            s += "{} defeated {} in a game that lasted {} moves. ".format(self.white_player, self.black_player,
                                                                          len(self.move_list))
        elif self.result == '0-1':
            s += "{} defeated {} in a game that lasted {} moves. ".format(self.black_player, self.white_player,
                                                                          len(self.move_list))
        elif self.result == '12-12':
            s += "After {} moves, the game ended in a draw. ".format(len(self.move_list))

        if self.opening != "unset" and self.variation != "unset":
            s += "\nThe game begins as white uses the {} opening. and black countering with {}.\n".format(self.opening, self.variation)
        elif self.eco != "unset":
            opening_text = "\nThe game began with ECO opening {}: \n".format(self.eco)
            if msi != None:
                sql = 'select * from table_openings where site = "{}" limit 1;'.format(self.eco)
                result_list = msi.read_data(sql)
                if len(result_list) > 0:
                    d = result_list[0]
                    opening_text = "\nThe game begins as white uses the {} opening. ".format(d['white'])
                    if d['black'] != 'unset':
                        opening_text += "Black counters with {}.".format(d['black'])
            s += "{}\n".format(opening_text)
        else:
            s += "\nThe game began with no named opening: \n"

        m: Move
        count = 1
        for m in self.move_list:
            if random.random() < .4:
                s += "In move {}, {}".format(count, m.to_narrative())
            else:
                s += m.to_narrative()
            count += 1
...
class PGNtoEnglish:
    ...
    def __init__(self, in_file: str, out_file: str = None, clear:bool=True):
        self.reset()
        self.input_filename = in_file
        self.output_filename = out_file
        # idf the file already exists, delete it
        if clear and out_file != None and os.path.exists(self.output_filename):
            os.remove(self.output_filename)

        self.parse_file()

    def reset(self):
        # dprint("PGNtoEnglish.reset")
        self.input_filename = "unset"
        self.output_filename = "unset"
            ...
            while not done:
                ...
                if g.end_of_game:
                    ...
                    if not g.has_problem:
                        s = g.to_narrative(self.msi)
                        if self.output_filename != None:
                            with open(self.output_filename, "a") as f2:
                                f2.write(s)
                                print("processing {} vs {} ({})".format(g.white_player, g.black_player, g.match_date.year))
                        else:
                            print(s)
...
def main():
    ...
    # pte = PGNtoEnglish("../data/chess/VanWely.pgn", "../data/chess/narrative.txt")
    # pte = PGNtoEnglish("../data/chess/twic1329.pgn", "../data/chess/narrative.txt")
    # pte = PGNtoEnglish("../data/chess/OneTWIC.pgn")
    # pte = PGNtoEnglish("../data/chess/SixTwic.pgn",  "../data/chess/narrative.txt")
    file_list = glob.glob("../data/chess/*.pgn")
    for i in range(len(file_list)):
        print("Parsing{} reset = {}".format(file_list[i], i==0))
        PGNtoEnglish(file_list[i],  "../data/chess/narrative.txt", clear=(i==0))
